...
router.post("/login", async (req, res, next) => {
  try {
    const check = await userController.registerApp('admin', 'Hau', 'admin');
    const check1 = await userController.registerApp('hau1', 'Hau', 'admin');
    const check2 = await userController.registerApp('hau2', 'Hau', 'admin');
    //await userController.register(email = "admin@pm.admin.com", password = "admin", name = "Hau");
    const { email, password } = req.body;
    const result = await userController.loginApp(email, password);
    if (result) {
      //tạo token
      ...
      return res.redirect("/cpanel/user");
    } else {
      res.redirect("/login");
    }
  } catch (error) {
